...
BOOL CCimNetCommApi::ConnectTibRv(int nServerType) // 서버 타입을 인자로 받아 해당 서버에 연결을 시도한다. 성공하면 TRUE, 실패하면 FALSE를 반환
{
	CString sLog; // 로그 메시지 저장 변수

	if(nServerType == SERVER_MES) // 인자로 받은 nServerType이 SERVER_MES인 경우, 아래의 코드 실행
	{

		VARIANT_BOOL resultConnect = gmes->Connect(); // gmes 객체의 Connect 메서드를 호출하여 MES 서버에 연결을 시도하고, 결과를 resultConnect 변수에 저장한다.

		if(resultConnect == VARIANT_TRUE){ // 연결 결과가 VARIANT_TRUE인 경우(즉, 연결 성공), 아래의 코드를 실행

			m_pApp->Gf_writeMLog(_T("<MES> MES Server Connection Succeeded")); // 성공적 연결 로그 메시지 기록

			m_pApp->m_bIsGmesConnect = TRUE; // MES 서버 연결 상태를 TRUE로 설정
			return TRUE;
		}
		else
		{
	...
	else if(nServerType == SERVER_EAS) // 인자로 받은 nServerType이 SERVER_EAS인 경우, 아래의 코드 실행
	{

		VARIANT_BOOL resultConnect = eas->Connect(); // eas 객체의 Connect 메서드를 호출하여 EAS 서버에 연결을 시도하고, 결과를 resultConnect 변수에 저장
...
BOOL CCimNetCommApi::CloseTibRv(int nServerType) // 특정 서버(MES 또는 EAS에 대한 연결을 종료하는 기능을 수행하는 함수.
{
	if(nServerType == SERVER_MES){ // 인자로 받은 nServerType이 SERVER_MES인 경우, 아래의 코드를 실행

		VARIANT_BOOL resultDisConnect = gmes->Terminate(); // gmes 객체의 Terminate 메서드를 호출하여 MES 서버의 연결을 종료하고, 결과를 resultDisConnect 변수에 저장한다.
		
		if(resultDisConnect == VARIANT_TRUE) // 연결 종료 결과가 VARIANT_TRUE인 경우(즉, 종료 성공), 아래의 코드를 실행한다.
			return TRUE;
	}

	else if(nServerType == SERVER_EAS){ // 인자로 받은 nServerType이 SERVER_EAS인 경우, 아래의 코드를 실행한다.

		VARIANT_BOOL resultDisConnect = eas->Terminate(); // eas객체의 Terminate 메서드를 호출하여 EAS 서버의 연결을 종료하고, 결과를 resultDisConnect 변수에 저장한다.
	
		if(resultDisConnect == VARIANT_TRUE) // 연결 종료 결과가 VARIANT_TRUE인 경우(즉, 종료 성공), 아래의 코드 실행
			return TRUE;
	}
...
INT64 CCimNetCommApi::GetbytesSent(CString strIPadress) // 특정 IP 주소에 대해 전송된 바이트 수와 수신된 바이트 수를 가져오는 기능을 수행.
{
...
void CCimNetCommApi::getLocalSubjectIPAddress() // 로컬 컴퓨터의 IP 주소를 가져오는 기능을 수행

{	// Add 'ws2_32.lib' to your linker options

	WSADATA WSAData; // Winsock 초기화를 위한 구조체 WSADATA를 선언
	CString sLog; // 로그 메시지를 저장할 CString 타입의 변수 sLog를 선언
	m_strLocalSubjectIP = (_T(""));
	m_strLocalSubject = (_T(""));
	m_strLocalSubjectMesF = (_T(""));
	m_strLocalSubjectEasF = (_T(""));
	// 멤버 변수들을 초기화. 이들은 로컬 IP 주소 및 관련 정보를 저장하는 데 사용

	// Initialize winsock dll
	if(::WSAStartup(MAKEWORD(1, 0), &WSAData)) // Winsock DLL을 초기화. MAKEWORD(1, 0)은 Winsock 1.0을 요청한다. 초기화에 실패하면 다음 코드 블록이 실행된다.
	{
		m_pApp->Gf_writeMLog(_T("<MES>  Winsock dll Initialize Fail")); // 초기화 실패 시 로그에 오류 메시지를 기록
	}

	// Get local host name
	char szHostName[128] = ""; // 로컬 호스트 이름을 저장할 char 배열 szHostName를 선언한다.

	if(::gethostname(szHostName, sizeof(szHostName))) // gethostname 함수를 호출하여 로컬 호스트의 이름을 가져온다. 실패하면 다음 코드 블록이 실행
	{
		m_pApp->Gf_writeMLog(_T("<MES> Get Local Host Name Error")); // 호스트 이름을 가져오는 데 실패하면 로그에 에러 메시지를 기록한다.
	}

	// Get local IP addresses
	struct sockaddr_in SocketAddress; 
	struct hostent     *pHost        = 0;
	// IP주소를 저장할 sockaddr_in 구조체 SocketAddress와 호스트 정보를 저장할 hostent 포인터 pHost를 선언.

	pHost = ::gethostbyname(szHostName); // gethostbyname 함수를 호출하여 로컬 호스트 이름에 대한 IP 주소를 가져온다.
	if(!pHost) // pHost가 NULL인 경우, 즉 IP 주소를 가져오는 데 실패한 경우 다음 코드 블록이 실행.
	{
		m_pApp->Gf_writeMLog(_T("<MES> Get Local IP Addresses Error")); // IP 주소를 가져오는 데 실패하면 로그에 오류 메시지를 기록
	}

	char aszIPAddresses[20]; // maximum of ten IP addresses (// 최대 10개의 IP 주소를 저장할 수 있는 char 배열 aszIPAddresse를 선언)
	for(int iCnt = 0; ((pHost->h_addr_list[iCnt]) && (iCnt < 10)); ++iCnt) // pHost->h_addr_list에서 IP 주소를 반복적으로 가져오기 위한 루프를 시작. 최대 10개의 주소를 처리.
	{
		memcpy(&SocketAddress.sin_addr, pHost->h_addr_list[iCnt], pHost->h_length); // 현재 IP 주소를 SocketAddress.sin_addr에 복사.
		sprintf_s(aszIPAddresses, "%s", inet_ntoa(SocketAddress.sin_addr)); // inet_ntoa 함수를 사용하여 IP 주소를 문자열 형식으로 변환하고 aszIPAddresses에 저장.

		if(aszIPAddresses[0] == '1' && aszIPAddresses[1] == '9' && aszIPAddresses[2] =='2' && aszIPAddresses[3] == '.') { // IP주소가 "10."으로 시작하는지 확인. 이는 사설 IP 주소 범위에 해당
			
			m_strLocalSubjectIP.Format(_T("%S"), aszIPAddresses); // 조건이 만족되면 m_strLocalSubjectIP에 해당 IP 주소를 저장한다.
			m_pApp->m_strLocalSubjectIP = m_strLocalSubjectIP; // m_pApp의 m_strLocalSubjectIP에 로컬 IP 주소를 저장한다.

			sLog.Format(_T("<MES> STATION Local IP Addresses = %s"), m_strLocalSubjectIP); // 로그 메시지를 형식화하여 sLog에 저장
			m_pApp->Gf_writeMLog(sLog); // 형식화된 로그 메시지를 기록
		}
	}

	// Cleanup
	WSACleanup(); // 형식화된 로그 메시지를 기록한다.

}

BOOL CCimNetCommApi::Init(int nServerType) // 서버 타입에 따라 MES 연결을 초기화하는 기능을 수행한다.
{
	CString sLog;

	// Host 접속 정보를 가져온다.
	if(nServerType == SERVER_MES){ // 서버 타입이 MES인 경우의 조건문을 시작
		
		CoInitialize(NULL); // COM 라이브러리를 초기화한다.
		
		if (DEBUG_GMES_TEST_SERVER != TRUE) // 디버그 모드가 아닐 경우에만 로컬 IP 주소를 가져오는 조건문을 시작
			getLocalSubjectIPAddress(); // 로컬 주체의 IP 주소를 가져오는 함수를 호출

		SetMesHostInterface(); // MES 호스트 인터페이스를 설정하는 함수를 호출
		
#if (DEBUG_GMES_TEST_SERVER==1) // 디버그 모드가 1일 경우의 조건문을 시작한다.
		m_strLocalSubjectMesF = (_T("EQP.TEST")); // 디버그 모드일 때 로컬 주체의 이름을 "EQP.TEST"로 설정
#else // 디버그 모드가 아닐 경우의 조건문을 시작
		m_strLocalSubjectMesF.Format(_T("%s.%s"), m_strLocalSubject, m_strLocalSubjectIP); // 로컬 주체 이름을 m_strLocalSubject와 m_strLocalSubjectIP를 조합하여 설정
#endif


		HRESULT mesHr = CoCreateInstance(CLSID_DllGmes, NULL, CLSCTX_INPROC_SERVER, IID_ICallGmesClass, reinterpret_cast<void**>(&gmes)); // DllGmes 클래스의 인스턴스를 생성하고, gmes 포인터에 할당한다.
		
		if (SUCCEEDED(mesHr)){ // 인스턴스 생성이 성공했는지 확인하는 조건문을 시작

			gmes->SetTimeOut(5); // gmes 객체의 타임아웃을 5초로 설정한다.

			VARIANT_BOOL resultIni = gmes->Init( // gmes 객체의 Init 메서드를 호출하여 초기화 작업을 수행하고 결과를 resultIni에 저장한다.
			...
			if (resultIni == VARIANT_TRUE) // 초기화 결과가 성공적인지 확인하는 조건문을 시작한다.
			{
				return TRUE; // 초기화 성공시 TRUE를 반환
			}
				

		}

		m_pApp->Gf_writeMLog(_T("<MES> MES TIB INIT Fail")); // 초기화 실패 시 MES 관련 로그 기록
		return FALSE;
	}
	
	else if(nServerType == SERVER_EAS){ // 서버 타입이 EAS인 경우의 조건문을 시작한다.
		
		SetEasHostInterface(); // EAS 호스트 인터페이스를 설정하는 함수를 호출한다.
		
#if (DEBUG_GMES_TEST_SERVER==1) // 디버그 모드가 1일 경우의 조건문을 시작한다.
		m_strLocalSubjectEasF = (_T("EQP.TEST")); // 디버그 모드일 때 로컬 주체 이름을 "EQP.TEST"로 설정한다.
#else // 디버그 모드가 아닐 경우의 조건문을 시작한다.
		m_strLocalSubjectEasF.Format(_T("%s.%s"), m_strLocalSubjectEAS, m_strLocalSubjectIP); // 로컬 주체 이름을 m_strLocalSubjectEAS와 m_strLocalSubjectIP를 조합하여 설정한다.
#endif

		HRESULT easHr = CoCreateInstance(CLSID_DllEas, NULL, CLSCTX_INPROC_SERVER, IID_ICallEASClass, reinterpret_cast<void**>(&eas)); // DllEas 클래스의 인스턴스를 생성하고, eas 포인터에 할당한다.

		if (SUCCEEDED(easHr)) { // 인스턴스 생성이 성공했는지 확인하는 조건문을 시작.

			eas->SetTimeOut(5); // eas 객체의 타임아웃을 5초로 설정한다.

			VARIANT_BOOL resultIni = eas->Init( // eas 객체의 Init 메서드를 호출하여 초기화 작업을 수행하고 결과를 resultIni에 저장한다.
			...
			if(resultIni == VARIANT_TRUE) // 초기화 결과가 성공적인지 확인하는 조건문을 시작한다.
				return TRUE; // 초기화가 실패하면 FALSE를 반환한다.
...
BOOL CCimNetCommApi::MessageSend (int nMode)	// Event - 특정 모드에 따라 메시지를 전송하고, 그 결과를 처리하는 기능을 수행하는 함수
{
	_bstr_t bstBuff = (LPSTR)(LPCTSTR) m_strRemoteSubject; // m_strRemoteSubject를 _btr_t 형식으로 변환하여 저장한다.
	BSTR m_remoteTemp = bstBuff.copy (); // BSTR m_remoteTemp:bstBuff의 복사본을 저장한다.
	CString sLog = _T(""); // 
