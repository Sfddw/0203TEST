...
void CHseAgingDlg::Lf_updateTowerLamp()
{
	...
	if (towerStatus != lpInspWorkInfo->m_nTowerLampStatus)
	{
		...
		{
			Lf_setDIOWrite(DIO_OUT_RED | DIO_OUT_BUZZER, DIO_OUT_RED_BLINK);
		}
		...
		{
			Lf_setDIOWrite(DIO_OUT_YELLOW, DIO_OUT_YELLOW_BLINK);
		}
	}
}
...
void CHseAgingDlg::Lf_writeSensingLog()
{
	...
	CTime time = CTime::GetCurrentTime();
	if ((time.GetMinute() - m_nSensingLogWriteMin) < lpSystemInfo->m_nSensingLogInterval)
	{
		return;
	}
	...
	if (fp == NULL)
	{
		...
		sprintf_s(buff, "TIME,RACK,LAYER,CH,PID,VCC,ICC,VBL,IBL\n");
		fprintf(fp, "%s", buff);
	}
	...
	for (int rack = 0; rack < MAX_RACK; rack++)
	{
		for (int layer = 0; layer < MAX_LAYER; layer++)
		{
			for (int ch = 0; ch < MAX_LAYER_CHANNEL; ch++)
			{
				...
				sprintf_s(buff, "%s,%d,%d,%d,%s,%.2fV,%.2fA,%.2fV,%.2fV",
					timeKey,
					rack + 1,
					layer + 1,
					ch + 1,
