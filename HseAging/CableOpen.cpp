...
void CCableOpen::OnTimer(UINT_PTR nIDEvent) // MFC 기반의 애플리케이션에서 타이머 이벤트를 처리하는 함수
{
	// TODO: 여기에 메시지 처리기 코드를 추가 및/또는 기본값을 호출합니다.
	if (nIDEvent == 1) // 타이머 이벤트 ID가 1인지 확인한다.
	{
		KillTimer(1); // 타이머 ID가 1인 타이머를 종료. 이로 인해 이후에 같은 타이머 이벤트가 발생하지 않도록 한다.

		// Cable Open Check 결과 값을 기준으로 Use/Unuse 채널 정보와 매칭검사 한다.
		if(m_pApp->pCommand->Gf_getCableOpenCheck(m_nRackNo) == TRUE) // Gf_getCableOpenCheck 함수를 호출하여 케이블 오픈 체크 결과를 가져온다. 결과가 TRUE인 경우 아래의 코드를 실행한다.
		{
			Lf_updateCableOpenResult(); // 케이블 오픈 결과를 업데이트하는 Lf_updateCableOpenRsult() 함수를 호출한다.
		}
		else // 만약 케이블 오픈 체크 결과가 TRUE가 아닌 경우 아래 코드 실행
		{
			for (int layer = 0; layer < MAX_LAYER; layer++) // MAX_LAYER만큼 반복하는 루프 시작
			{
				for (int ch = 0; ch < MAX_LAYER_CHANNEL; ch++) // 각 레이어 내에서 MAX_LAYER_CHANNEL만큼 반복하는 루프 시작
				{
					m_pSttCableOpen[layer][ch]->SetWindowText(_T("Comm NG")); // 현재 레이어와 채널에 해당하는 UI 요소와 텍스트를 "Comm NG"로 설정. 이는 통신 오류를 나타내는 메시지
				}
			}
			Lf_refreshCableOpen(); // 케이블 오픈 상태를 새로 고치는 Lf_refreshCableOpen 함수를 호출
		}
	}
	CDialog::OnTimer(nIDEvent);
}

void CCableOpen::OnBnClickedMbcCoRetry() // 버튼 클릭 이벤트를 처리하는 함수 (Retry 버튼 클릭 시)
{
	...
	Lf_refreshCableOpen(); // 케이블 오픈 상태를 고치는 Lf_refreshCableOpen 함수를 호출
	SetTimer(1, 100, NULL);
}
