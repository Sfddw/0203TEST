...
BOOL CUDPSocket::CreatSocket(UINT nSocketPort, int nSocketType) // UDP 소켓 생성, 브로드 캐스트 옵션을 설정하는 함수
{
	CString m_stError; // 오류 메시지를 저장하기 위한 문자열 변수

	ZeroMemory(m_sendBuf, sizeof(m_sendBuf)); // 버퍼초기화 (소켓을 통해 전송할 데이터를 저장하는데 사용)

	//소켓을 생성
	if (!Create(nSocketPort, nSocketType)) // Create 함수를 호출하여 소켓을 생성. 소켓 생성에 실패하면 FALSE를 반환
	{ // 소켓 생성이 실패하면
		m_stError.Format(_T("Socket Create Fail -> %d"), GetLastError()); // 실패했을 경우 오류 메시지를 포맷하여 m_stError에 저장. GetLastError()는 마지막 오류 코드를 반환
		AfxMessageBox(m_stError); // 오류 메시지를 사용자에게 보여주는 메시지 박스를 표시
		return FALSE;
	}
	...
	int flag=TRUE;
	int len, rflag;
	// 브로드캐스트 옵션을 설정하기 위한 변수, flag는 브로드캐스트를 활성화하기 위한 플래그이다.

	if (!SetSockOpt(SO_BROADCAST, &flag, sizeof(int))) // 소켓 옵션을 설정하는 SetSockOpt 함수를 호출하여 브로드캐스트 옵션을 활성화한다. 실패할 경우 FALSE를 반환한다.
	{
		m_stError.Format(_T("SetSockOpt failed to set SO_BROADCAST:% d"), GetLastError()); // 실패했을 경우, 오류 메시지를 포맷하여 m_stError에 저장한다.
		AfxMessageBox (m_stError); // 오류 메시지를 사용자에게 보여주는 메시지 박스를 표시한다.

		return FALSE;
	}

	len = sizeof(rflag); // rflag의 크기를 len에 저장. 이는 GetSockOpt에서 사용 될 것이다.
	if (!GetSockOpt(SO_BROADCAST, &rflag, &len) && rflag == 0) // 브로드캐스트 옵션이 제대로 설정되었는지 확인하기 위해 GetSockOpt를 호출한다. 설정이 실패하거나 rflag가 0이면 (브로드캐스트가 비활성화된 경우)다음 코드를 실행
	{
		m_stError.Format(_T("GetSockOpt failed to get SO_BROADCAST:% d"), GetLastError()); // 브로드캐스트 옵션을 가져오는 데 실패했을 경우, 오류 메시지를 포맷하여 m_stError에 저장
		AfxMessageBox (m_stError); // 오류 메시지를 사용자에게 보여주는 메시지 박스를 표시
